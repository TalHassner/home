...
3. Open a shell (e.g xterm)
    ...
    cd renderer
4. Edit ldlibMatlab.bash and set LIBS_PREFIX to the download folder, e.g.
    LIBS_PREFIX=/home/user/download
...
INSTALLATION
---------------
The code was tested on MATLAB releases 2009b - 2013a on Linux. However there are no Linux dependencies in the code so minor changes could be made to make it run on Windows.
...
A pre-compiled version of the mex file for Linux 64-bit, renderer.mexa64 is provided. Also, a pre-compiled version of the required libraries,  renderer-pre-compiled-libs.tar.gz, can be downloaded separately from our site:
https://www.openu.ac.il/home/hassner/projects/poses/
...
COMPILATION
---------------  
A pre-compiled version of the mex file, renderer.mexa64 is already provided for Linux 64-bit. It is best to try to use it first. Please refer to the FAQ section for troubleshooting.
...
>> mex renderer.cpp depth.cpp Engine.cpp EngineOSG.cpp util/util.cpp -lGL -lX11 -losg -losgViewer -losgDB -losgGA -losgUtil -lOpenThreads -lGLU -Iutil/ -L../local/lib64 -I../local/include
...
A: sudo does not pass the LD_LIBRARY_PATH variable. If you have root access then simply edit /etc/ld.so.conf.d (or equivalent) with the correct dependent libraries folders.
...
A: Currently there isn't. However, there are no Linux dependencies in the code so it might compile and link on Windows with minor modifications. Please let us know if succeed in doing so.
