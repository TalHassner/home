function [flo,wrp,clrflo] = compute_dense_flow(Im_Source, Im_Target)
% demo.m provided as a function. 
% Computes dense flow between two images using the 
% Scale-less SIFT (SLS) descriptors [1]. The demo will compute flow from 
% im1 (source, or "Left" image) to im2 (target, or "Right" image).
%
% Notes: 
...
%		available on your MATLAB path in order for this demo to work.
%	2. To compute dense correspondences this code uses the SIFT-Flow algorithm available from [2]
%		It MUST be installed and available on your MATLAB path in order for this demo to work.
%	3. This code will reproduce the results reported in the paper. For the optimized descriptor
%		(not described in the paper) please see the help for extract_scalelessdescs
%
%  References:
...
% 
%
%   The SOFTWARE ("extract_scalelessdescs" and "compute_dense_flow") is provided "as is", without any
...
im_src=imread(Im_Source);
im_tgt=imread(Im_Target);
